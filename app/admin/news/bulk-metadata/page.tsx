...
export default function BulkMetadataNewsPage() {
  ...
  async function run(selectedMode: Mode) {
    if (selectedMode === "regenerate-all") {
      const confirmed = confirm(
        "This regenerates SEO/metadata for every news article, overwriting anything already set. This can't be undone. Continue?"
      );
      if (!confirmed) return;
    }
    ...
    let query = supabase
      .from("news")
      .select(
        "id, title, excerpt, cover_image_url, meta_title, meta_description, og_title, og_description, og_image_url"
      );
    ...
    for (let i = 0; i < rows.length; i += BATCH_SIZE) {
      ...
      const results = await Promise.all(
        batch.map(async (row) => {
          // Same fill logic as the Prompt Library's bulk
          // metadata tool: each field keeps whatever it already
          // has unless empty, or — in "regenerate all" mode — is
          // always replaced with the article's own content.
          const metaTitle =
          ...
          const ogDescription =
            selectedMode === "regenerate-all"
              ? row.excerpt || ""
              : row.og_description || row.excerpt || "";
          const ogImageUrl =
            selectedMode === "regenerate-all"
              ? row.cover_image_url || ""
              : row.og_image_url || row.cover_image_url || "";
    ...
    <main className="min-h-screen bg-white px-6 py-10 text-zinc-900">
      <div className="mx-auto max-w-2xl">
        ...
        <p className="mt-1 text-sm text-zinc-600">
          ...
          article&apos;s own title, excerpt, and cover image — applied across
        ...
        <div className="mt-6 space-y-3">
          ...
          <div className="rounded-2xl border border-red-200 bg-red-50 p-4">
            ...
            <p className="mt-1 text-xs text-zinc-600">
              Overwrites SEO/metadata for every article, even ones that
              already have it set. Requires confirmation.
            </p>
            <button
              type="button"
              disabled={running}
        ...
        {(running || finished) && (
          <div className="mt-6 rounded-2xl border border-zinc-200 bg-zinc-50 p-4">
            {running && (
              <>
                <p className="text-sm text-zinc-700">
                  {mode === "regenerate-all" ? "Regenerating" : "Generating"}{" "}
                  metadata... {done} / {total}
                </p>
            ...
            {finished && (
              <p className="text-sm text-zinc-700">
                {total === 0
                  ? "Nothing to do — every article already has complete metadata."
